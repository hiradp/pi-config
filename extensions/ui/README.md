...
The Pi header centers a stencil logo above a compact loadout summary: slime green in dark mode, darker olive in light mode. `/loadout` opens the full resource names in a scrollable overlay; Escape closes it. Loading errors and conflicts remain in the startup transcript.
...
`palette.ts` defines the working-area colors separately from the rest of the theme, with dark and light variants (including `rustic-light`). Theme switching updates the header, prompt, and footer without a reload.
