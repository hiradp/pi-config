...
2. Use one `subagent` call in parallel mode to invoke `code-reviewer` three times:
   - model `openai-codex/gpt-6-astra`, label `Correctness · Astra`, with primary focus `correctness/completeness`
   - model `fireworks/accounts/fireworks/models/kimi-k3`, label `Correctness · Kimi K3`, with primary focus `correctness/completeness`
   - model `openai-codex/gpt-6-astra`, label `Simplicity · Astra`, with primary focus `simplicity`
3. Start each delegated task with `Review authorization: /review-code` and its exact `Primary focus:` value. Give each isolated reviewer the target, original intent and requirements, relevant scope context, and prior findings when present. Do not tell any reviewer about the others' output. Keep the simplicity pass scoped to changed code and context needed to validate concrete simplification candidates, rather than repeating the full correctness checklist. For rollout-, version-skew-, feature-flag-, or configuration-sensitive targets, ask the correctness reviewers for proportionate coverage of reachable old/new component and configuration combinations during partial rollout, rollback and flag-disable behavior, and malformed, missing, mixed-provider, or other schema-admitted configurations; do not impose a generic exhaustive matrix on unrelated reviews.
4. The tool reports a child as failed when it exits abnormally, times out, returns no final response, or answers `Unsupported task:`. If any pass failed, identify each failed pass and stop without retrying, synthesizing findings, or saying `No confirmed findings.`
5. After all three succeed, verify every alleged finding and redundant-test candidate against the current code and review target. Deduplicate overlapping results, discard ungrounded claims and style preferences, and preserve meaningful disagreements as questions.
6. Report confirmed findings first, ordered by severity, and identify which reviewer or reviewers (model and focus) raised each one. Then report questions, prior-finding dispositions, testing gaps, redundant tests, and review coverage when applicable. Keep removable low-value tests in the redundant-tests section rather than inflating them into defects. In review coverage, record the reviewed revision or working-tree state, confirm that all three independent passes returned usable output, and state whether confirmed findings remain. If there are no confirmed defects, say `No confirmed findings.` even when a redundant-tests section follows.
