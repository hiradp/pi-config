---
description: Review the current implementation plan independently with Astra and Grok 4.6
argument-hint: "[additional instructions]"
---

Review authorization: /review-plan

Run exactly one read-only review pass over the latest complete implementation plan in this conversation. The line above is what lets the `subagent` tool dispatch `plan-reviewer`: it must be in the current user message, and it covers one `subagent` call. A second call, or a call under a later message, is refused.
...
2. Use one `subagent` call in parallel mode to invoke `plan-reviewer` twice:
   ...
   - model `xai/grok-4.6`, label `Plan review · Grok 4.6`
